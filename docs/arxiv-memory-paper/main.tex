...
% ============================================================
\begin{abstract}
Sixty percent of context tokens in current LLM agentic sessions are wasted -- redundant re-explanation of decisions already made in prior sessions.
Baseline measurements from 304 production sessions confirm a median bootstrap cost of 30{,}115 input tokens with a 60\% context waste ratio, while existing memory systems~\citep{packer2023memgpt, park2023generative, chhikara2025mem0} organize around dialogue episodes and code-RAG~\citep{zhang2023repocoder, wang2024coderagbench} retrieves without decision provenance, leaving this waste unaddressed.

We present a \emph{three-layer workflow memory} architecture -- domain, workflow, and practitioner layers with distinct retrieval semantics -- targeting ${\leq}10$K bootstrap tokens and ${\leq}20\%$ waste.
The key architectural primitive is \emph{dual-mode retrieval}: pull mode queries all layers at session start; push mode refreshes memory for dormant tasks proportional to repository activity, so that context remains current even after weeks of inactivity.
The third contribution reframes the memory layer as scalable oversight infrastructure: \emph{retrieval-correction edits} -- practitioner corrections that would have been unnecessary had memory surfaced relevant context -- constitute a process-level oversight signal that is denser than outcome-level supervision and scales with agent autonomy~\citep{ovcharov2026recursive}.

The architecture is deployed on a production legal-technology platform (70+ MCP tools, 380M+ records, 1{,}547 merged PRs in 105 days).
The memory layer does not merely consume alignment data -- it produces it: every retrieval failure that triggers a practitioner correction simultaneously generates a preference pair for the companion alignment pipeline~\citep{ovcharov2026recursive}.
\end{abstract}
...
Push mode runs as a scheduled orchestrator -- a separate Docker container with a Bedrock Claude Haiku summarization pipeline.
...
\begin{itemize}[leftmargin=*, nosep]
  \item An \textbf{executive summary} of changes relevant to the task since the last refresh, generated by Bedrock Claude Haiku from the diff of relevant commits and issue comments.
...
The technology stack reuses production infrastructure: Qdrant for vector storage, PostgreSQL for structured metadata (new \texttt{workflow\_memory\_*} tables), Redis for caching, and AWS Bedrock Claude Haiku for summarization steps.
...
All seven phases (0, 1.0--1.5) have been implemented and deployed.
The complete implementation spans three PostgreSQL migrations (\texttt{144}--\texttt{146}), three Qdrant vector collections (\texttt{wm\_domain}, \texttt{wm\_workflow}, \texttt{wm\_practitioner}), and six MCP tools (\texttt{workflow\_memory\_query}, \texttt{workflow\_memory\_ingest}, \texttt{workflow\_memory\_stats}, \texttt{workflow\_memory\_reconcile}, \texttt{workflow\_memory\_push\_sync\_tasks}, \texttt{workflow\_memory\_push\_refresh}).
The phase schedule and implementation details are available in the supplementary materials.\footnote{Phase schedule: \url{https://github.com/overthelex/SecondLayer/blob/main/docs/workflow-memory-phases.md}}
...
Four metrics operationalize the architecture's success criteria, each paired with a baseline measurement method and a target (Table~\ref{tab:metrics}).
...
\captionof{table}{Evaluation metrics with measured baselines and targets.
  Bootstrap baselines are measured from 304 Claude Code sessions in the companion paper's transcript dataset.}
\label{tab:metrics}
\vspace{0.3em}
\begin{tabular}{@{}p{2.5cm} p{3.5cm} p{2.8cm} p{2.8cm}@{}}
\toprule
\textbf{Metric} & \textbf{Measurement method} & \textbf{Baseline} & \textbf{Target} \\
\midrule
Bootstrap token cost &
  Input tokens at first API call (incl.\ \texttt{CLAUDE.md} cache); $N{=}304$ &
  30{,}115 (median) & ${\leq}10$K \\[4pt]
Context waste ratio &
  Files read but never edited\,/\,total files read; $N{=}180$ with ${\geq}3$ reads &
  60\% (median) & ${\leq}20\%$ \\[4pt]
Principle retrieval accuracy &
  Held-out principle ledger entries; queries from edit-traces &
  0.604 (top-1 cos.\ sim.) & ${\geq}0.80$ (top-1 cos.\ sim.) \\[4pt]
...
The memory layer's primary reduction target is therefore the automatic context cost ($T$), not the file-read count (Figure~\ref{fig:bootstrap-cost}).

\begin{figure}[t]
\centering
\input{figures/fig_bootstrap_cost}
\caption{Bootstrap token cost distribution across 304 sessions (median 30{,}115; P10/P90: 18{,}540/41{,}774). The dashed line marks the target (${\leq}10{,}000$ tokens) after memory-enabled retrieval replaces unconditional context loading.}
\label{fig:bootstrap-cost}
\end{figure}
...
Line count is noisier ($R^2 = 0.58$) due to periodic reformatting that changes density without changing content volume (Figure~\ref{fig:claudemd-growth}).

\begin{figure}[t]
\centering
\input{figures/fig_claudemd_growth}
\caption{\texttt{CLAUDE.md} size over 85 days (solid) with linear projection (dashed). At the observed growth rate of 217~chars/day, context cost compounds linearly with project age.}
\label{fig:claudemd-growth}
\end{figure}
...
Waste ratio correlates negatively with session length: short sessions (5--10 turns) waste 68.5\%; longer sessions ($>$50 turns) waste 51.1\% (Figure~\ref{fig:waste-breakdown}).
...
\begin{figure}[t]
\centering
\input{figures/fig_waste_breakdown}
\caption{Context waste ratio by file type and session length. Source-code reads are wasted at 78\%; longer sessions waste less (51\% vs.\ 69\%). The dashed line marks the target (${\leq}20\%$).}
\label{fig:waste-breakdown}
\end{figure}

Phase~0 (prompt-commit bridge) has been deployed since May~9, 2026 with negligible performance overhead (15ms per capture); the corpus contains 533 prompts across 13 qualifying sessions (${\geq}5$ prompts each) at time of writing.

\paragraph{Deployment state (all phases complete).}
All seven phases (0, 1.0--1.5) are deployed on the local environment as of May~13, 2026.
The memory layer contains 187 entries across all three layers: 170 domain principles (140 from \texttt{CLAUDE.md} sections, 23 from merged PR descriptions, 7 from design documents), 4 workflow patterns (push-refresh summaries generated via Bedrock Claude Haiku), and 13 practitioner session summaries (covering 533 captured prompts across 13 qualifying sessions).
All embeddings use Amazon Titan Embed Text v2 (1024 dimensions) via AWS Bedrock (Figure~\ref{fig:memory-composition}).

\begin{figure}[t]
\centering
\input{figures/fig_memory_composition}
\caption{Memory layer composition as of May~13, 2026 (187 entries). Domain principles dominate; workflow and practitioner layers are in early accumulation.}
\label{fig:memory-composition}
\end{figure}
...
The reconciliation tool (Phase~1.4) was tested on a session that modified 3 files (\texttt{deployment/docker-compose.local.yml}, \texttt{mcp\_backend/src/services/workflow-memory-service.ts}, \texttt{mcp\_backend/src/migrations/145*.sql}).
Of the 170 principles, 113 were identified as relevant by tag-based and keyword-based matching; the session had retrieved only 1 principle, yielding precision${}=1.0$ and recall${}=0.009$.
The low recall is expected: this session had only one \texttt{workflow\_memory\_query} call.
The 112 missed principles were correctly flagged as retrieval-correction candidates.

Push-mode (Phase~1.5) has been deployed with the Plane task watcher, LLM-based summarization (Bedrock Claude Haiku), and tool lineage delta detection.
Four active tasks are on the watchlist; the initial tool lineage snapshot covers 22 registered tools.
The first push-refresh cycle completed successfully, generating 4 workflow patterns with executive summaries and open questions at a total LLM cost of \$0.005.

Full A/B evaluation of bootstrap reduction (memory-enabled vs.\ memory-disabled sessions) requires several weeks of operational data and will be reported in a subsequent revision. Figure~\ref{fig:metrics-summary} summarizes the gap between measured baselines and targets across all four metrics.

\begin{figure}[t]
\centering
\input{figures/fig_metrics_summary}
\caption{Summary of evaluation metrics: measured baselines (blue) versus targets (green), normalized so that each baseline equals 100\%. For bootstrap tokens, context waste, and refresh latency, lower is better; for retrieval accuracy, higher is better. The gap between baseline and target quantifies the reduction the memory layer must achieve.}
...
This is the same constraint as the companion paper~\citep[\S8]{ovcharov2026recursive}.
...
All seven phases (0, 1.0--1.5) have been implemented and deployed on the local environment as of May~13, 2026.
Baseline measurements from 304 sessions confirm a median bootstrap cost of 30{,}115 input tokens and a context waste ratio of 60\%; the memory layer targets ${\leq}10$K tokens and ${\leq}20\%$ waste, with ${\geq}0.80$ top-1 cosine similarity on principle ledger entries.
