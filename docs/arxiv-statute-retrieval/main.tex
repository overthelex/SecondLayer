...
\title{Temporal Decay of Co-Citation Predictability:\\A 20-Year Statute Retrieval Benchmark\\from 502M Ukrainian Court Citations}

\author{
  Volodymyr Keshchenko\\
  SecondLayer\\
  \texttt{mcvovkes@gmail.com}
}
...
We test this assumption longitudinally by constructing UA-StatuteRetrieval, a benchmark that measures co-citation predictability across 20 annual snapshots (2007--2026) of 502 million citation records from 126 million Ukrainian court decisions.
...
From the Ukrainian Unified State Register of Court Decisions (\foreignlanguage{ukrainian}{ЄДРСР}, 126M decisions, 2005--2026), we extract 502 million statutory citations and construct annual retrieval benchmarks spanning 20 years.
...
We process 126 million full-text decisions partitioned by adjudication year.
...
After cleaning (retaining only well-formed article references), we obtain 502 million citation records.
...
For criminal procedure, static co-citation indexes remain viable; for civil and administrative law, text-based retrieval (dense embeddings, BM25 over article text) is necessary to maintain quality.
The domain-specific decay rates enable targeted hybrid strategies.
...
The underlying 502M citation records remain proprietary; the co-citation graph is available separately~\citep{keshchenko2025citation}.
...
\begin{enumerate}[nosep,leftmargin=*]
  ...
  \item \emph{Graph-only baselines}: text-based retrievers (likely more temporally robust) are not evaluated. This is deliberate -- establishing graph-method decay is a prerequisite for motivating text-based alternatives.
...
These findings challenge the implicit assumption that citation structure provides stable retrieval signal.
Legal information systems that rely on co-citation should implement continual graph updates, and the mid-frequency / civil-law tail should be served by complementary text-based retrieval.
The released benchmark enables direct measurement of temporal robustness for any retrieval system.
