...
\paragraph{Mitigation: exponential recency weighting.}
Rather than the binary choice of including or excluding historical data, we propose weighting each case's contribution to the co-citation matrix by recency: $w(\text{case}) = \exp(-\lambda \cdot \Delta t)$, where $\Delta t$ is the age in years and $\lambda$ controls the decay rate.
Table~\ref{tab:reweighting} shows that this principled approach consistently outperforms uniform weighting (3--23\% improvement), with the optimal $\lambda$ decreasing over time: $\lambda = 1.0$ is best for 2014--2018 (aggressive downweighting of pre-reform data), while $\lambda = 0.1$--$0.2$ is optimal for 2022--2024 (where post-reform data already dominates).
This adaptive behavior makes exponential reweighting preferable to sliding windows, which require choosing a fixed cutoff: reweighting smoothly discounts stale signal rather than discarding it entirely.

\begin{table}[t]
\centering
\small
\caption{Exponential recency weighting: MRR (Adamic-Adar) with $w = \exp(-\lambda \cdot \Delta t)$. $\lambda = 0$ is uniform (baseline). Bold: best $\lambda$ per year.}
...
Exponential recency weighting provides a principled, parameter-light mitigation: a single $\lambda$ parameter yields 3--23\% improvement over uniform weighting, with the optimal value shifting from aggressive ($\lambda = 1.0$) to mild ($\lambda = 0.1$) as post-reform data accumulates.
