...
The decay is non-uniform: criminal procedure maintains stable co-citation patterns (MRR $\sim$0.40), while civil law degrades from 0.35 to 0.15 following the 2017 judicial reform.
...
These findings establish that graph-based legal retrieval requires continual updating and that neural text-based methods are necessary precisely where co-citation signal is weakest.
...
These findings have direct implications for legal retrieval system design: graph-based methods should not be treated as static indexes but require continual updating, and neural text-based retrieval is necessary specifically for the mid-frequency, civil/administrative tail where co-citation fails.
...
\begin{tabular}{@{}l ccc@{}}
\toprule
\textbf{Metric} & \textbf{Adamic-Adar} & \textbf{Common Neighbors} & \textbf{Degree} \\
\midrule
Hit@1  & 0.145 & 0.141 & 0.030 \\
Hit@5  & 0.406 & 0.398 & 0.063 \\
Hit@10 & 0.545 & 0.534 & 0.111 \\
Hit@20 & 0.675 & 0.664 & 0.175 \\
MRR    & 0.272 & 0.266 & 0.059 \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\small
\caption{BM25 (case text $\to$ article text) vs.\ co-citation (Adamic-Adar). BM25 uses masked case text (citations stripped) as query against article full texts. Both methods degrade over time.}
\label{tab:bm25}
\begin{tabular}{@{}l cc cc@{}}
\toprule
\textbf{Year} & \textbf{AA MRR} & \textbf{BM25 MRR} & \textbf{AA Hit@10} & \textbf{BM25 Hit@10} \\
\midrule
2008 & 0.409 & 0.068 & 0.721 & 0.112 \\
2012 & 0.509 & 0.022 & 0.789 & 0.037 \\
2016 & 0.369 & 0.027 & 0.647 & 0.047 \\
2020 & 0.330 & 0.052 & 0.619 & 0.091 \\
2024 & 0.324 & 0.047 & 0.607 & 0.082 \\
\midrule
\textbf{Decay} & \textbf{20.8\%} & \textbf{31.1\%} & & \\
\bottomrule
\end{tabular}
\end{table}
...
However, the two signals are complementary: co-citation excels on high-frequency articles with dense graph structure, while BM25 provides nonzero signal for rare articles where co-citation fails entirely.
...
\caption{Per-codex MRR over time. Criminal Procedure remains stable; Civil Code degrades most. The 2017 reform creates a visible inflection in civil/administrative codes.}
\label{fig:codex}
\end{figure}
...
\begin{itemize}[nosep,leftmargin=*]
  ...
  \item \textbf{Civil Code} (0.35 $\to$ 0.15): steepest decline, reflecting evolving interpretive practice and the 2017 reform's reorganization of civil procedure.
...
(1)~\emph{Structural breaks} -- the 2017 judicial reform replaced old procedural codes with new ones, disrupting established co-citation patterns. This manifests as a visible inflection in Figure~\ref{fig:codex}.
...
The decay rate of $\sim$3\% MRR/year for mid-frequency articles implies that a graph-based retrieval system loses meaningful performance within 3--4 years without retraining.
...
The decay is genuine (persists on fixed articles, strengthens without data leakage), non-uniform (concentrates in civil/administrative domains and mid-frequency articles), and practically significant ($\sim$3\% MRR/year loss for the articles that matter most).
