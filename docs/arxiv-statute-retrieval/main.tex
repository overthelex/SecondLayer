...
Civil Procedure articles exhibit the highest drift (median 0.055), followed by Criminal Code (0.045), while Administrative Offenses articles are most stable (0.032).
This ordering mirrors the per-codex MRR trajectories in Figure~\ref{fig:codex}: the domains with highest semantic drift are precisely those with steepest retrieval degradation.
...
(1)~\emph{Semantic drift} -- the legal context in which articles are cited evolves over time (mean 4.3\% embedding shift per 12 years), weakening both co-citation and lexical retrieval signals (Section~\ref{fig:drift}).
...
BM25 text retrieval decays even faster (31\%), and embedding drift analysis reveals that the underlying cause is semantic evolution: the legal context in which articles are cited shifts 4.3\% over 12 years, with civil procedure drifting twice as fast as criminal law.

These findings challenge the assumption that legal retrieval signals -- whether structural or textual -- are temporally stable.
