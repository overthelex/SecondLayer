...
The author's prior work~\citep{ovcharov2025citation,ovcharov2025statute} documented an empirical case that sharpens this problem.
...
The five informal conditions from the author's prior work~\citep{ovcharov2025citation,ovcharov2025statute} are expressed as general concept inclusions (GCIs) in \DL, with a defined concept $\mathsf{ValidOversight}$ that is the conjunction of all five.
...
This level is directly relevant to the human--LLM oversight regime documented in the author's prior work~\citep{ovcharov2025citation,ovcharov2025statute}.
...
The author~\citep{ovcharov2025citation,ovcharov2025statute} challenged this assumption empirically.
...
Persistent state ensures each correction is informed by the cumulative history of prior agent behavior and its consequences~\citep{ovcharov2025citation,ovcharov2025statute}.
\end{axiom}
...
\end{enumerate}
This workflow is precisely the LEX~AI case study documented in the author's prior work~\citep{ovcharov2025citation,ovcharov2025statute}, augmented with an ontology-driven prompt generation layer.
\end{proof}
...
Figure~\ref{fig:radar} visualizes this gap across all five conditions.

\begin{figure}[t]
\centering
\input{figures/fig_radar_comparison}
\caption{Radar comparison of condition satisfaction: Domain Constitution (blue, all five conditions) vs.\ OntoChatGPT (red, C1 and C3 partially). The Domain Constitution strictly subsumes OntoChatGPT's control and adds three new dimensions.}
\label{fig:radar}
\end{figure}
...
The oversight ontology\footnote{Ontology and session-level dataset available at \url{https://huggingface.co/datasets/overthelex/oversight-constitution}; source code at \url{https://github.com/overthelex/SecondLayer}.} is authored in OWL~2~DL Manchester syntax.
...
We instantiate the ontology with individuals derived from the LEX~AI production dataset~\citep{ovcharov2025citation,ovcharov2025statute}: 2{,}892 workflow sessions, 30{,}510 edit pairs, and 1{,}579 attributed outcomes collected over 105~days.
...
Classification results for the full ABox (Figure~\ref{fig:oversight-dist}):
...
\begin{figure}[t]
\centering
\input{figures/fig_oversight_distribution}
\caption{Distribution of 2{,}892 sessions by oversight grade. $\mathsf{PartialOversight}$ dominates (68.1\%); only 0.8\% of sessions satisfy all five conditions for $\mathsf{FullOversight}$.}
\label{fig:oversight-dist}
\end{figure}
...
C5 (consequential grounding) at 54.6\% matches the outcome attribution coverage from the pilot dataset~\citep{ovcharov2025citation,ovcharov2025statute}.
...
Figure~\ref{fig:condition-heatmap} summarizes the condition satisfaction landscape across oversight tiers.

\begin{figure}[t]
\centering
\input{figures/fig_condition_heatmap}
\caption{Condition satisfaction by oversight tier. C2 (compositional layering) is the dominant bottleneck: satisfied in only 19.4\% of sessions. C4 (information asymmetry) is near-universal (94.3\%).}
...
Figure~\ref{fig:funnel} shows how the 2{,}892 sessions are progressively filtered through each condition.

\begin{figure}[t]
\centering
\input{figures/fig_condition_funnel}
\caption{Session attrition through the five oversight conditions. C2 (compositional layering) is the dominant bottleneck, reducing 2{,}892 sessions to 24 $\mathsf{FullOversight}$ instances. C4 (information asymmetry) is near-universal.}
\label{fig:funnel}
\end{figure}
...
The result is counterintuitive: $\mathsf{FullOversight}$ sessions have a \emph{lower} positive outcome rate (76.5\%) than $\mathsf{PartialOversight}$ sessions (96.5--97.0\%), as shown in Figure~\ref{fig:outcome-rates}.
This is not a failure of the constitution but a validation of it.

\begin{figure}[t]
\centering
\input{figures/fig_outcome_rates}
\caption{Positive outcome rate by oversight grade ($N = 1{,}391$ strong-confidence attributions). $\mathsf{FullOversight}$ ($\gamma=5$) has a lower success rate than $\mathsf{PartialOversight}$ because fully instrumented sessions tackle structurally harder compositional tasks.}
\label{fig:outcome-rates}
\end{figure}
...
$\mathsf{FullOversight}$ sessions exhibit a distinctive edit profile (Figure~\ref{fig:edit-profile}): \emph{lower} substantive rewrite rate (53.8\% vs.\ 78--83\%) but \emph{higher} rejection rate (15.4\% vs.\ 2.5--4.7\%).

\begin{figure}[t]
\centering
\input{figures/fig_edit_profile}
\caption{Edit behavior across oversight tiers. $\mathsf{FullOversight}$ sessions have fewer substantive rewrites but 3--6$\times$ higher rejection rates -- concentrating the most informative oversight signal.}
\label{fig:edit-profile}
\end{figure}
This is consistent with Experiment~3 from the author's prior work~\citep{ovcharov2025citation,ovcharov2025statute}, which found that rejection (binary halt of the agentic trajectory) correlates with 78\% positive outcomes -- the highest of any edit class.
...
Three findings from the empirical validation connect to the main paper's experiments (Figure~\ref{fig:rejection-outcome} visualizes the first):

\begin{figure}[t]
\centering
\input{figures/fig_rejection_outcome}
\caption{Rejection rate vs.\ positive outcome rate by oversight tier. $\mathsf{FullOversight}$ sessions (highest rejection rate) tackle structurally harder tasks, resulting in lower but more meaningful success rates. Point size reflects session count.}
\label{fig:rejection-outcome}
\end{figure}
