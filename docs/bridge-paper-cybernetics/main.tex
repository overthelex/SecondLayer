...
OntoChatGPT satisfies 2 of 5 conditions (C1 fully, C3 partially), confirming the strict specialization result (Proposition~\ref{prop:non-subsumption}): $\mathsf{ValidOversight}$ inherits C1 and C3 and adds C2, C4, C5.
...
The ten atomic concepts from Definition~\ref{def:signature} map directly to OWL named classes.
...
Four individuals instantiate the invalid patterns from Proposition~\ref{prop:negative}:
...
\midrule
\texttt{oneshot\_script}    & One-shot code generation      & C1, C2 \\
...
We use HermiT~\citep{glimm2014hermit} via \texttt{owlready2}~0.50 (Python OWL~API with embedded HermiT reasoner) for all verification tasks.
...
All four negative individuals are correctly classified, confirming Proposition~\ref{prop:independence}.
