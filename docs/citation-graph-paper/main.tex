...
\author{
  Volodymyr Ovcharov\textsuperscript{1} \and
  Oleksandr V.\ Palagin\textsuperscript{2}
  \\[6pt]
  \textsuperscript{1}LEX AI LLC, Kyiv, Ukraine \\
  \textsuperscript{2}V.M.\ Glushkov Institute of Cybernetics, NAS of Ukraine, Kyiv \\[4pt]
...
A regex-based extraction pipeline running on the production database identifies six citation types---codex article references, named law references, constitutional references, inter-case references, law-by-number references, and supreme court ruling references---yielding approximately \textbf{[N\_TOTAL]} million citation edges.
...
This ontology is operationalized as the domain layer of a workflow memory system for LLM-assisted legal analysis~\citep{ovcharov2026workflowmemory}, connecting citation-derived structure to the ontology-controlled paradigm~\citep{palagin2006architecture,palagin2023ontochatgpt}.

The extraction pipeline, graph analysis code, and aggregated statistics are released as open data.
The full-text corpus is publicly accessible via the EDRSR API~\citep{edrsr2024}.
...
\textbf{[PLACEHOLDER --- awaiting pipeline completion.]}

Expected structure:
\begin{itemize}[leftmargin=*, nosep]
  \item Total citations extracted (by type)
  \item Citations per decision (mean, median, distribution)
  \item Temporal trends: citations per decision by year
  \item Processing time and throughput
\end{itemize}

% Preliminary from 2006 partition (8,547 decisions):
% 30,580 citations, 3.58 per decision
% codex_article: 27,697 (90.6%), law_article: 1,740 (5.7%),
% case_reference: 688 (2.2%), constitution: 239 (0.8%),
% law_by_number: 121 (0.4%), supreme_court_ruling: 95 (0.3%)
...
\textbf{[PLACEHOLDER --- awaiting extraction completion.]}

Expected structure:
\begin{itemize}[leftmargin=*, nosep]
  \item Bipartite graph statistics: $|D|$, $|L|$, $|E|$, density
  \item Degree distribution (legislation side): power-law fit
  \item Hub legislation articles: top-20 most cited
  \item Temporal evolution of citation density
\end{itemize}
...
Expected structure:
\begin{itemize}[leftmargin=*, nosep]
  \item Number of communities, modularity score
  \item Largest communities mapped to legal domains
  \item Inter-community citation patterns
  \item Comparison with manual legal domain classification
\end{itemize}
...
\textbf{[PLACEHOLDER --- manual annotation of random sample.]}

A random sample of 200 decisions will be manually annotated to measure extraction precision and recall per citation type.
...
We presented the first large-scale citation graph constructed from the complete Ukrainian court decision registry---100.7 million decisions, 99.5 million full texts, [N\_TOTAL] million citation edges.
Three results emerge.

First, regex-based citation extraction at the $10^8$-decision scale is practical on commodity hardware: the full pipeline completes in [HOURS] hours on a single 4-vCPU server, demonstrating that industrial-scale legal NLP does not require specialized infrastructure.
