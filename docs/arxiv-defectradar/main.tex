...
\begin{abstract}
We introduce \emph{legislative definition quality assessment} (\textsc{LegDefQA}) -- the task of automatically detecting logical defects in statutory definitions -- and present \textsc{DefectRadar}, the first NLP pipeline for this task.
Applied to 5{,}799 definitions from 44{,}021 active Ukrainian legislative acts, the pipeline flags 32.4\% as morphologically circular (\emph{circulus in definiendo}) and 32.5\% as relying on cross-law undefined terms (\emph{ignotum per ignotum}).
However, error analysis using a RAG-augmented GPT-4o judge reveals that \textbf{85\% of flagged circulus cases are benign domain terminology reuse}, not normative tautology -- yielding an estimated true defect rate of $\sim$3\%.
For ignotum, 71\% are cross-reference gaps (terms defined in other laws) and 21\% are critical (undefined anywhere in the corpus).
These findings transform DefectRadar from a binary defect detector into a \emph{severity-aware triage tool}: the $\sim$10\% of definitions with severe defects warrant legislative attention, while the remainder represent systematic but lower-priority drafting gaps.
Prior work has addressed definition extraction~\citep{hosabettu2025statutory}, stylistic quality~\citep{hofler2012style}, and definition generation~\citep{chouhan2024lexdrafter}, but no system has attempted to detect logical defects within definitions -- a task that \citet{hofler2012style} identified as an open problem 14 years ago.
We release the \textsc{LegDefQA} benchmark (train/dev/test splits), pipeline code, and 5{,}799 annotated definitions at \url{https://huggingface.co/datasets/overthelex/ua-defectradar}.
\end{abstract}

\noindent\textbf{Keywords:} legal NLP, legislative definition quality assessment, definitional defects, \emph{circulus in definiendo}, \emph{ignotum per ignotum}, Ukrainian law, morphological analysis, cross-legislative definition graph, benchmark
...
Tier 1: Morphological circulus & 1{,}880 (32.4\%) \\
Tier 2: Semantic circulus ($\tau = 0.90$) & 24 (0.4\%) \\
\midrule
Combined circulus & 1{,}904 (32.8\%) \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:circulus} reports detection results on the full corpus.
The morphological detector (Tier~1) identifies 1{,}880 definitions (32.4\%) where a discriminating adjective or qualifier from the \emph{definiendum} reappears in the \emph{differentia specifica}.

The semantic detector (Tier~2) applies multilingual E5-large to the 3{,}919 definitions that passed Tier~1 (no morphological overlap).
The cosine similarity between the embedded \emph{definiendum}+\emph{genus} and the embedded \emph{differentia} has a median of 0.827 ($\sigma = 0.031$) across the corpus -- reflecting the inherent semantic alignment between a term and its definition.
At the conservative threshold $\tau = 0.90$ (top 0.6\% of the distribution), Tier~2 identifies 24 additional definitions as semantically circular -- cases where the \emph{definiens} is a near-verbatim restatement or semantic paraphrase of the \emph{definiendum} without sharing surface morphemes.
For example, \foreignlanguage{ukrainian}{``виробник вина''} (wine producer) defined as \foreignlanguage{ukrainian}{``виробник продукту виноградарства, виробник продукту виноробства''} (producer of viticultural product, producer of winemaking product) -- a semantic paraphrase at cosine similarity 0.921.
The low yield (24 vs.\ 1{,}880 from Tier~1) confirms that morphological overlap is the dominant circulus mechanism in Ukrainian legislative definitions; semantic-only circularity is rare.
Figure~\ref{fig:semantic-dist} shows the similarity distribution with the threshold.

\begin{figure}[t]
\centering
\input{figures/fig4_semantic_dist.tex}
\caption{Cosine similarity distribution for 3{,}919 Tier-1-negative definitions (E5-large). The median (0.827) reflects inherent alignment between term and definition. Threshold $\tau=0.90$ captures 24 semantic paraphrases in the far-right tail.}
\label{fig:semantic-dist}
\end{figure}
...
Figure~\ref{fig:temporal} shows defect prevalence by enactment period.
Both defect types maintain high prevalence across the entire temporal span: \emph{circulus} ranges from 28--37\% and \emph{ignotum} from 24--36\% in most periods, indicating that definitional defects are a \emph{persistent structural feature} of Ukrainian legislative drafting rather than a recent phenomenon.
The pre-2004 laws (3{,}092 definitions from the old numbering system) show defect rates comparable to recent legislation, suggesting that neither the shift to Roman-numeral convocations nor EU harmonization fundamentally changed the drafting quality of definitions.

\begin{figure}[t]
\centering
\input{figures/fig2_temporal.tex}
\caption{Defect prevalence by enactment period. Both defect types remain high (28--37\%) across all periods, indicating a persistent structural feature of legislative drafting.}
\label{fig:temporal}
\end{figure}

\paragraph{Per-law distribution.}
Figure~\ref{fig:domain} shows defect prevalence for the top-11 laws by definition count (collectively containing 1{,}026 definitions, 17.7\% of the corpus).

\begin{figure}[t]
\centering
\input{figures/fig3_domain.tex}
\caption{Defect prevalence in the top-11 laws by definition count. Payment services law (1591-IX) exhibits the highest \emph{ignotum} rate (83.0\%), while electronic communications (1089-IX) has the highest \emph{circulus} rate among single-domain laws (44.9\%).}
\label{fig:domain}
\end{figure}

The payment services law (1591-IX) stands out with 83.0\% \emph{ignotum} rate -- the vast majority of its 88 definitions rely on terms defined only in other laws (financial instruments, clearing, settlement).
Electronic communications (1089-IX) has the highest \emph{circulus} rate among single-domain laws (44.9\%), with 31 of 69 definitions repeating qualifying adjectives.
Notably, its \emph{ignotum} rate is unusually low (7.2\%), because this law defines its own comprehensive terminology internally -- the opposite pattern from payment services, which imports terms extensively.
...
\subsection{Error Analysis and Severity Stratification}
\label{sec:error-analysis}

To assess whether pipeline-flagged defects represent genuine legislative quality problems, we conduct three targeted experiments using the RAG-augmented LLM judge on independent samples of 100 definitions each.

\paragraph{Experiment A: Circulus classification ($n=33$).}
We submit 100 pipeline-flagged \emph{circulus} definitions and ask the LLM to classify each as: \textsc{True\_Defect} (normatively empty, adds no discriminative content), \textsc{Domain\_Reuse} (root repeats but definition is substantively meaningful), or \textsc{False\_Positive} (no actual overlap).

\begin{table}[t]
\centering
\caption{Error analysis of pipeline-flagged \emph{circulus} definitions.}
\label{tab:error-circulus}
\begin{tabular}{lrr}
\toprule
\textbf{Classification} & \textbf{Count} & \textbf{\%} \\
\midrule
\textsc{Domain\_Reuse} & 28 & 84.8 \\
\textsc{True\_Defect} & 3 & 9.1 \\
\textsc{False\_Positive} & 2 & 6.1 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:error-circulus} reveals that \textbf{84.8\% of pipeline-flagged circulus cases are domain terminology reuse} -- the morphological root recurs across \emph{definiendum} and \emph{differentia}, but the definition is substantively meaningful.
For example, \foreignlanguage{ukrainian}{``страховий продукт''} (insurance product) defined through \foreignlanguage{ukrainian}{``страхової послуги''} (insurance service) shares the root \foreignlanguage{ukrainian}{``страх-''}, but the definition properly distinguishes product from service through genus-differentia structure.
Only 9.1\% are genuinely normatively empty definitions.
This means the pipeline's \emph{raw} 32.4\% circulus rate should be understood as a \emph{morphological overlap rate}; the \emph{substantive defect rate} is approximately $32.4\% \times 9.1\% \approx 3\%$ of all definitions.

\paragraph{Experiment B: Ignotum severity ($n=24$ terms from 15 definitions).}
We submit 100 pipeline-flagged \emph{ignotum} definitions and ask the LLM to classify each undefined term as: \textsc{Critical} (term undefined anywhere, norm inapplicable), \textsc{Cross\_Ref} (defined in another law, needs explicit reference), or \textsc{Common} (well-known, no definition needed).

\begin{table}[t]
\centering
\caption{Severity stratification of \emph{ignotum} flagged terms.}
\label{tab:ignotum-severity}
\begin{tabular}{lrr}
\toprule
\textbf{Severity} & \textbf{Count} & \textbf{\%} \\
\midrule
\textsc{Cross\_Ref} & 17 & 70.8 \\
\textsc{Critical} & 5 & 20.8 \\
\textsc{Common} & 2 & 8.3 \\
\bottomrule
\end{tabular}
\end{table}

Figure~\ref{fig:error-analysis} visualizes the classification and severity distributions.

\begin{figure}[t]
\centering
\input{figures/fig5_error_analysis.tex}
\caption{Error analysis results. (a)~84.8\% of pipeline-flagged circulus cases are domain terminology reuse; only 9.1\% are true normative defects. (b)~70.8\% of ignotum-flagged terms are cross-reference gaps (defined in other laws); 20.8\% are critical (undefined anywhere).}
\label{fig:error-analysis}
\end{figure}

The majority of ignotum flags (70.8\%) represent \textsc{Cross\_Ref} gaps -- terms that \emph{are} defined in other laws but not in the current one.
These are genuine drafting deficiencies (the reader must know which other law to consult) but not normative vacuums.
The 20.8\% \textsc{Critical} cases -- terms with no statutory definition anywhere in the corpus -- represent the most actionable findings for legislative reform.

\paragraph{Experiment C: False negatives ($n=12$ from 100 pipeline-clean).}
We submit 100 definitions that the pipeline classified as defect-free and ask the LLM to independently assess them.
The LLM identifies \emph{circulus} in 66.7\% and \emph{ignotum} in 100\% of these ``clean'' definitions, confirming that the pipeline is conservative by design: it catches clear morphological overlaps but misses semantic paraphrases and compound-derivation circularity.
The high LLM positive rate on clean definitions ($\gg$50\%) reinforces our earlier finding that the LLM over-flags, but also establishes a pipeline false-negative rate of $\sim$67\% for circulus -- a known trade-off of the morphological approach that could be addressed by implementing the semantic Tier~2 detector (Section~\ref{sec:circulus-method}).
...
The pipeline flags 52.0\% of all statutory definitions as exhibiting at least one formal defect.
Our error analysis (Section~\ref{sec:error-analysis}) reveals that this headline number requires stratification.
For \emph{circulus}, 84.8\% of flagged cases are \emph{domain terminology reuse} -- morphologically circular but substantively meaningful -- yielding an estimated \emph{true defect rate} of $\sim$3\% (approximately 170 definitions corpus-wide).
These are the normatively empty provisions that \citet{kyrychenko2026creative} showed can have real budgetary consequences.
For \emph{ignotum}, 70.8\% of flagged terms are \emph{cross-reference gaps} (defined in other laws but not in the current one) and 20.8\% are \emph{critical} (undefined anywhere in the corpus).
The raw 52\% thus decomposes into: $\sim$3\% severe circulus + $\sim$7\% critical ignotum + $\sim$23\% cross-reference gaps requiring explicit citations + $\sim$27\% domain reuse requiring no action.
This stratification transforms DefectRadar from a binary defect detector into a \emph{triage tool}: the 10\% of definitions with severe or critical defects warrant immediate legislative attention, while the cross-reference gaps ($\sim$23\%) represent a systematic but lower-priority drafting improvement.
...
% ============================================================
\section{The LegDefQA Benchmark}
\label{sec:benchmark}
% ============================================================

To facilitate future work on legislative definition quality assessment, we release \textsc{LegDefQA} -- a benchmark derived from the DefectRadar corpus.

\paragraph{Task.}
Given a legal definition (definiendum + definiens + source law), predict: (a)~whether the definition exhibits \emph{circulus in definiendo} (binary), and (b)~which terms in the definiens constitute \emph{ignotum per ignotum} cross-reference gaps (sequence labeling).

\paragraph{Data splits.}
We partition the 5{,}799 definitions into train (3{,}522 from 527 laws), dev (1{,}153 from 176 laws), and test (1{,}124 from 176 laws) with \textbf{no law overlap} between splits -- ensuring that models cannot memorize law-specific patterns.
The circulus positive rate is balanced across splits (31.8\%/34.4\%/32.3\%); ignotum is similarly stable (31.6\%/32.6\%/35.3\%).

\paragraph{Baselines.}

\begin{table}[t]
\centering
\caption{\textsc{LegDefQA} benchmark baselines on the test set ($n = 1{,}124$).}
\label{tab:benchmark}
\small
\begin{tabular}{lR{1cm}R{1cm}R{1cm}R{1cm}R{1cm}R{1cm}}
\toprule
& \multicolumn{3}{c}{\textbf{Circulus}} & \multicolumn{3}{c}{\textbf{Ignotum}} \\
\cmidrule(lr){2-4} \cmidrule(lr){5-7}
\textbf{System} & P & R & F1 & P & R & F1 \\
\midrule
Majority (all YES) & 32.3 & 100 & 48.8 & 35.3 & 100 & 52.2 \\
Random (matched) & 35.7 & 35.3 & 35.5 & 35.6 & 35.3 & 35.4 \\
GPT-4o RAG judge & 59.6 & 96.9 & 73.8 & 33.9 & 100 & 50.6 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:benchmark} reports baseline results on the test split.
The GPT-4o RAG judge achieves 73.8\% F1 on circulus (high recall, moderate precision) but performs at majority-class level on ignotum (F1 50.6 vs.\ 52.2) -- the near-universal flagging rate (100\% recall) makes the ignotum task particularly challenging for discriminative approaches.
The DefectRadar pipeline defines the ground-truth labels and thus cannot be evaluated against itself; future work should establish human-annotated gold labels on the test set to enable pipeline evaluation.
...
We present representative examples from the error analysis (Section~\ref{sec:error-analysis}) to illustrate the distinction between true defects and domain terminology reuse.

\subsection*{A.1 True Defects (Normatively Empty)}

\paragraph{Example 1.}
\textbf{Law:} 894-2025-\foreignlanguage{ukrainian}{п} (IT systems regulation) \\
\textbf{Term:} \foreignlanguage{ukrainian}{програмно-апаратна помилка} (software-hardware error) \\
\textbf{Definition:} \foreignlanguage{ukrainian}{помилка, недолік або дефект у програмному забезпеченні або програмних складових частинах, програмно-апаратному устаткованні...} \\
\textbf{Defect:} The qualifier \foreignlanguage{ukrainian}{``програмно-апаратна''} reappears verbatim in the differentia. The definition does not establish any criterion for distinguishing software-hardware errors from other types of errors -- it merely restates the term.

\paragraph{Example 2.}
\textbf{Law:} z1124-25 (securities regulation) \\
\textbf{Term:} \foreignlanguage{ukrainian}{тимчасовий керівник виконавчого органу підсанкційного професійного учасника} \\
\textbf{Definition:} \foreignlanguage{ukrainian}{особа, призначена НКЦПФР тимчасовим керівником виконавчого органу підсанкційного професійного учасника...} \\
\textbf{Defect:} The entire multi-word term is repeated in the definiens without genus or differentia. The definition provides no information beyond restating the definiendum with the addition of the appointing authority.

\paragraph{Example 3.}
\textbf{Law:} 1145-2022-\foreignlanguage{ukrainian}{п} (vehicle registry) \\
\textbf{Term:} \foreignlanguage{ukrainian}{електронна заява} (electronic application) \\
\textbf{Definition:} \foreignlanguage{ukrainian}{заява про внесення до Реєстру відомостей, яка формується через електронний кабінет...} \\
\textbf{Defect:} The root \foreignlanguage{ukrainian}{``електрон-''} from the definiendum reappears as \foreignlanguage{ukrainian}{``електронний кабінет''} in the differentia; the definition fails to explain what makes the application ``electronic'' beyond pointing to an ``electronic cabinet.''

\subsection*{A.2 Domain Terminology Reuse (Not Defective)}

\paragraph{Example 4.}
\textbf{Law:} 1089-IX (electronic communications) \\
\textbf{Term:} \foreignlanguage{ukrainian}{канал кабельної каналізації електронних комунікаційних мереж} \\
\textbf{Definition:} \foreignlanguage{ukrainian}{окремо виділені місця обмеженої ємності в інфраструктурі кабельної каналізації, призначені для прокладання кабелів...} \\
\textbf{Classification:} Domain reuse. The genus (``designated spaces of limited capacity in infrastructure'') provides substantive content; the repetition of ``cable duct'' terminology is inherent to the domain.

\paragraph{Example 5.}
\textbf{Law:} 2469-IX (medicines) \\
\textbf{Term:} \foreignlanguage{ukrainian}{Державний реєстр лікарських засобів} (State Register of Medicines) \\
\textbf{Definition:} \foreignlanguage{ukrainian}{єдина державна інформаційна система, що містить відомості про зареєстровані лікарські засоби...} \\
\textbf{Classification:} Domain reuse. ``Medicines'' necessarily appears in both the term and its definition; the genus (``unified state information system'') properly distinguishes the register from other uses of the term.

\paragraph{Example 6.}
\textbf{Law:} 2804-IX (chemical safety) \\
\textbf{Term:} \foreignlanguage{ukrainian}{оцінка безпечності хімічної речовини} (chemical substance safety assessment) \\
\textbf{Definition:} \foreignlanguage{ukrainian}{аналіз інформації щодо небезпечних властивостей хімічної речовини, визначення безпечних рівнів впливу...} \\
\textbf{Classification:} Domain reuse. The root \foreignlanguage{ukrainian}{``безпеч-''} recurs in ``safe exposure levels'' -- a distinct technical concept within the chemical safety domain.
