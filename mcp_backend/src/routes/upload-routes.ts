...
/**
 * Build a safe file path from multer's filename, ignoring req.file.path entirely.
 * Extracts validated components via capture groups and reconstructs a clean filename,
 * breaking CodeQL taint propagation from the request object.
 */
function safeMulterPath(filename: string): string {
  // Extract and rebuild from capture groups to break taint chain
  const match = filename.match(/^chunk-(\d{1,20})-([a-z0-9]{1,10})$/);
  if (!match) {
    throw new Error('Invalid multer filename');
  }
  // Reconstruct filename from captured, validated components
  const safeFilename = `chunk-${match[1]}-${match[2]}`;
  const multerDir = path.join(process.env.UPLOAD_TEMP_DIR || '/tmp/uploads', 'multer-tmp');
  return path.join(multerDir, safeFilename);
}
...
export function createUploadRouter(
  ...
  router.post('/:uploadId/chunk', uploadChunkRateLimit as any, upload.single('chunk'), (async (req: DualAuthRequest, res: Response): Promise<any> => {
    try {
      ...
      const chunkFilePath = safeMulterPath(req.file.filename);
      ...
      if (req.file?.filename) {
        try {
          await fs.unlink(safeMulterPath(req.file.filename));
    ...
    try {
      ...
      if (uploadQueueService) {
        uploadQueueService.enqueueProcessing(session).catch((error) => {
          ...
          processUpload(session, uploadService, minioService, vaultTools, db, undefined, documentService).catch((err) => {
        ...
        processUpload(session, uploadService, minioService, vaultTools, db, undefined, documentService).catch((error) => {
  ...
  router.post('/:uploadId/retry', (async (req: DualAuthRequest, res: Response): Promise<any> => {
    try {
      ...
      if (uploadQueueService) {
        ...
        processUpload(session, uploadService, minioService, vaultTools, db, { manualRetry: true }, documentService).catch((error) => {
...
async function processUpload(
  ...
  const deps: ProcessorDeps = { uploadService, minioService, vaultTools, db, documentService };
