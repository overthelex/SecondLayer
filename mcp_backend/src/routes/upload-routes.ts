 ...
 * Multer filename is generated by our callback (chunk-{ts}-{random}), so validate it.
 */
function safeMulterPath(filename: string): string {
  // Only allow our generated filenames: chunk-{digits}-{alphanumeric}
  if (!/^chunk-\d+-[a-z0-9]+$/.test(filename)) {
    throw new Error('Invalid multer filename');
  }
  const multerDir = path.join(process.env.UPLOAD_TEMP_DIR || '/tmp/uploads', 'multer-tmp');
  return path.join(multerDir, filename);
}
