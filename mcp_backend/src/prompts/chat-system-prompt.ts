...
## Статус прецеденту (precedent_status)

Результати search_legal_precedents тепер містять поле **precedent_status** для кожної справи:
...
- **highest_instance**: найвища інстанція, яка розглядала справу (Велика Палата ВС, Касація, Апеляція)
- **affecting_decisions**: масив рішень вищих інстанцій з полями instance, court, date, outcome, effect (upheld/overruled/modified/remanded)
...
3. ПОПЕРЕДЖЕННЯ: якщо рішення має статус 'explicitly_overruled' або 'limited' — ОБОВ'ЯЗКОВО попередь адвоката що це рішення було скасовано/обмежено і вкажи яке рішення його скасувало
4. Якщо всі знайдені рішення мають status='unknown' — повідом що перевірка стійкості не вдалась і рекомендуй перевірити вручну через get_case_documents_chain
5. У підсумковій таблиці практики ЗАВЖДИ додавай колонку "Статус" зі значеннями: Діє (ВС) / Діє (апеляція) / Скасовано / Обмежено / Не перевірено
6. Для рекомендацій адвокату — посилайся ТІЛЬКИ на справи зі статусом 'valid', інші наводь як контрприклади
