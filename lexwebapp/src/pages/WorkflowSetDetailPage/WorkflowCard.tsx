...
const STATUS_BADGE_KEYS: Record<string, { key: string; color: string; icon: typeof CheckCircle }> = {
  pending: { key: 'status.pending', color: 'bg-gray-100 text-gray-600', icon: Clock },
  ...
  cancelled: { key: 'status.cancelled', color: 'bg-gray-100 text-gray-500', icon: Square },
};
    ...
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
      <div className="p-5">
        ...
        {workflow.cost_usd > 0 && (
          <div className="text-xs text-gray-400 mb-3">
            {t('workflowCard.cost')}: {formatUah(workflow.cost_usd)}
          </div>
        )}
        ...
        <div className="flex items-center justify-between">
          ...
          >
            ...
            {workflow.plan.steps.length} {t('workflowCard.steps')}
          </button>
      ...
      {expanded && (
        <div className="border-t border-gray-100 bg-gray-50 px-5 py-4">
          <h5 className="text-xs font-semibold text-gray-500 uppercase mb-3">{t('workflowCard.executionPlan')}</h5>
