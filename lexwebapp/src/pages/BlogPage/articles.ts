...
## Додаток А: Результати Phase 0 — перевірка здійсненності

*Додано 8 травня 2026 — емпірична валідація методології на реальних продакшн-даних.*

Ретроспективна екстракція Phase 0 була виконана на кодовій базі LEX AI, щоб оцінити чи щільність сигналу від рекурсивних робочих процесів достатня для переходу до Phase 1. Пайплайн (модуль \`rlhf-signals/\`, [вихідний код](https://github.com/overthelex/secondlayer)) обробив три джерела даних: GitHub Pull Requests (GraphQL API), задачі Plane (REST API) та сесії Claude Code (локальні JSONL-файли).

### Загальні показники

| Метрика | Значення |
|---------|----------|
| Сесій workflow | 2 892 |
| Артефактів | 33 402 |
| Пар редагувань | 30 510 |
| Атрибутованих outcome | 1 411 |
| Сер. артефактів на сесію | 11.5 |

### Сесії за джерелом

| Джерело | Сесій | Артефактів | Редагувань | Покриття outcome |
|---------|-------|------------|------------|-----------------|
...
\\*Транскрипти Claude Code не мають зовнішніх шляхів атрибуції outcome. Зв'язок транскрипт→PR — задача Phase 1.

### Класифікація редагувань

Усі 30 510 пар редагувань класифіковані двофазним підходом: rule-based межі (cosmetic < 5% нормалізованої відстані, substantive ≥ 80%) + LLM-класифікація (AWS Bedrock, Claude Sonnet 4.6) для середнього діапазону. Покриття класифікації: 99.96%.

| Семантичний клас | К-сть | % |
|-----------------|-------|---|
...
Домінування substantive rewrites (80.7%) підтверджує ключову гіпотезу: рекурсивні робочі процеси фаундера генерують переважно змістовні правки, а не косметичні. Показник rejection 3.6% — це випадки повного відхилення LLM-виводу, цінний негативний сигнал для preference learning.

### Атрибуція outcome

| Тип outcome | К-сть | Впевненість |
|------------|-------|-------------|
| Merged clean (>30д, без revert) | 801 | Strong |
| Issue resolved | 508 | Strong |
| Follow-up fix PR | 70 | Medium |
| Closed unmerged | 22 | Strong |
| Closed obsolete | 10 | Strong |

96% outcome атрибутовані з high confidence. Вікна атрибуції: 504 outcome протягом 8–30 днів, 877 протягом 31–90 днів.

### Зв'язок транскриптів з PR

Транскрипти Claude Code — найбагатше джерело (26.8 артефактів/сесію), але спочатку мали 0% покриття outcome, бо транскрипти не мають зовнішніх ідентифікаторів. Ми зв'язали їх з GitHub PR через temporal proximity: для кожного транскрипту SecondLayer знайшли найближчий PR, де час початку транскрипту потрапляв у активне вікно PR (created − 4h до merged + 1h).

| Впевненість зв'язку | К-сть | Критерій |
|---------------------|-------|----------|
| Strong | 252 | < 1 години |
| Medium | 133 | 1–4 години |
| Weak | 83 | 4+ годин |

468 з 473 транскриптів SecondLayer успішно зв'язані. Outcome PR пропаговані на зв'язані транскрипти з пониженням впевненості для weak links.

**Вплив на датасет:**

| Метрика | До linking | Після linking | Зміна |
|---------|-----------|--------------|-------|
| Claude Code outcome coverage | 0.0% | 16.0% | +16pp |
| Usable preference pairs | 2 668 | **5 354** | **+101%** |
| Deep edit sessions з outcomes | 146 | **276** | **+89%** |
| Total outcomes | 1 411 | **1 579** | +168 |

### Вердикт

Ключова метрика з Розділу 3.3 методології — сесії з не-косметичними правками та атрибутованими outcome за місячне вікно:

| Період | Сесій |
|--------|-------|
| Найкращий місяць (березень 2026) | **788** |
| Середнє за місяць | **282** |
| Всього | **1 579** |

Поріг GO встановлено на рівні ≥50 сесій/місяць. Фактична щільність сигналу 788 сесій у найкращий місяць перевищує поріг у **15.8 разів**. Зв'язок транскриптів з PR подвоїв кількість usable preference pairs до 5 354.

**Вердикт: GO — переходимо до Phase 1 live capture.**

Ретроспективних даних достатньо для початкових експериментів з preference model — 5 354 usable preference pairs з усіх трьох джерел даних.
