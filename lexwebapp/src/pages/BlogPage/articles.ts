...
Тут починається найцікавіша частина. Коли моніторинг-сторінка пішла на прод, ми отримали:
...
Останній етап -- перенести 30M+ записів з локальної бази на продакшн. Скрипт \`copy-fulltext-to-prod.py\` працює у дві фази:

**Фаза 1: Export.** Локальний PG експортує дані в TSV-чанки на NVMe-диск. Швидке послідовне читання, без навантаження на мережу.

**Фаза 2: Upload.** 200 паралельних воркерів завантажують чанки на прод через \`COPY FROM STDIN\` + \`INSERT ON CONFLICT\`. Шардинг по \`doc_id\`: записи з \`doc_id < 112M\` йдуть в основну базу, решта -- в \`secondlayer_prod_ft2\`.

Прогрес виводиться кожні 200 чанків: кількість скопійованих, пропущених (вже існують), помилок, швидкість, ETA.

## Результати

| Метрика | Значення |
|---------|----------|
| Повних текстів | 30,400,000 |
| Розмір даних | 137 ГБ |
| Кількість судів | 685 |
...
Повні тексти -- це сировина. Наступний крок -- побудова векторних ембедінгів для семантичного пошуку по 30 мільйонах рішень. Це окрема інженерна задача з власними масштабними проблемами.

А поки що -- pipeline працює, дані на проді, моніторинг показує 94-97% покриття, і PostgreSQL більше не падає від shared memory exhaustion.

---

*Відкриті дані -- це не компроміс. Це принцип. 30 мільйонів судових рішень доступні кожному громадянину, і ми побудували інфраструктуру, щоб зробити їх по-справжньому доступними для аналізу.*`,
  },
  {
