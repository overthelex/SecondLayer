...
export const articles: Article[] = [
  {
    ...
    punchline: 'ЄДРСР — вся судова практика України у відкритому доступі. Зараз у проді крутиться векторизація останньої великої когорти — 33.7M цивільних справ через Voyage AI voyage-3.5. Розбираємо пайплайн: чанкінг, паралелізм, checkpoint/resume, прод-інцидент із postgres OOM, і скільки це коштує.',
    category: 'tech',
...
`,
  },
  {
    ...
    title: 'Як я написав 735 комітів за 25 днів: Claude Code як повноцінний інженерний напарник',
    punchline: '486 сесій, 5 612 повідомлень, 193 000 рядків коду, 22 326 bash-команд — і жодного найнятого розробника. Реальна статистика використання Claude Code для побудови legal tech платформи, data pipelines на 340M+ записів та MCP-інтеграцій.',
    category: 'tech',
    tags: ['Claude Code', 'AI', 'Productivity', 'Startups', 'DevOps', 'MCP'],
    readTime: '12 хв',
    publishedAt: '2026-04-12',
    content: `# Як я написав 735 комітів за 25 днів: Claude Code як повноцінний інженерний напарник
...
## Цифри за 25 днів (18 березня — 12 квітня 2026)

| Метрика | Значення |
|---------|----------|
| Сесій | 486 (285 проаналізовано) |
| Повідомлень | 5 612 |
| Комітів | 735 |
| Рядків написано | +193 340 |
| Рядків видалено | -14 259 |
| Файлів змінено | 1 811 |
| Bash-команд | 22 326 |
| Edit-операцій | 3 782 |
| Sub-агентів | 864 |
| Паралельних сесій | 41% повідомлень |

Це не теоретична продуктивність. Це реальний git log.

---

## Що саме я будував

### 1. Legal Tech платформа (28 сесій)

Основний продукт: баг-фікси, нові фічі (Diia-автентифікація, контракти розробників, email-нотифікації, іспанська локалізація з geo-detection), UI-редизайн, 93+ тестів.

Claude Code працював як full-stack розробник: мультифайлові зміни, створення PR, мердж, деплой — все в одній сесії.

### 2. Data pipelines для відкритих даних (18 сесій)
...
### 3. Інфраструктура та DevOps (16 сесій)

EC2 provisioning в різних регіонах (Париж, Іспанія), CI/CD pipeline фікси, blue-green деплой з preview-середовищем, Docker/nginx дебагінг, міграції серверів. Claude Code використовував AWS MCP tools для прямого провіжнінгу інфраструктури.

### 4. MCP Server Ecosystem (14 сесій)

Побудова та конфігурація MCP-серверів для Nextcloud Deck/Tables, Thunderbird email та ChatGPT. Міграція 180 тасок з Linear в Nextcloud Deck. Синхронізація 402 issues.

### 5. Контент та side-проєкти (8 сесій)

Інвестор-мемо, юридичні документи, блог-статті, офіційні листи, та... Telegram-бот з цитатами Бендера з Футурами з мультимовною базою даних.

---

## Як виглядає типова робоча сесія

Я не пишу детальні промпти. Мій стиль — **запускаю Claude на задачу, дивлюсь що робить, коригую курс в реальному часі**.

Статистика підтверджує: 106 випадків коли Claude починав з неправильного підходу, які я швидко перенаправляв. Це не баг — це особливість роботи з AI-напарником. Як з джуніор-розробником, який дуже швидко вчиться.
...
**Результат: 89% сесій завершились successfully** (mostly + fully achieved).
...
Коли потрібно змінити тип в shared пакеті, оновити backend handler, frontend компонент та тести одночасно — Claude Code робить це за одну ітерацію. Для людини це 30-60 хвилин переключення контексту. Для Claude — одна команда.

### Дебагінг — 25 сесій

Claude читає логи, знаходить root cause, пропонує фікс. Не завжди з першого разу — але значно швидше ніж гуглити Stack Overflow.

### MCP-інтеграції як операційна інфраструктура

Я з'єднав Claude Code з:
- **Nextcloud Deck** — таск-менеджмент прямо з терміналу
- **AWS API** — провіжнінг EC2, security groups, EIP без виходу з IDE
- **Thunderbird** — email-менеджмент через MCP
- **Кастомні MCP-сервери** — для ChatGPT-інтеграції
...
**Приклад:** для repartitioning великої PostgreSQL-таблиці Claude пробував batch DELETE+INSERT, screen-сесії які вмирали, дедлоки — поки я не підказав bulk INSERT + TRUNCATE.

### Buggy Code — 102 випадки

Код з першого разу працює не завжди. Type errors, missing imports, неправильні SQL-запити. Але з TypeScript та тестами це ловиться швидко.

### Марафонські сесії — більше помилок

Сесії на 10+ різних задач накопичують більше помилок. Фокусовані сесії на 2-3 задачі стабільно досягають повного результату.
...
| Якість | 89% success rate | Залежить від команди |
...
### 1. CLAUDE.md — це ваш онбординг-документ

Замість пояснювати кожну сесію «ми використовуємо PostgreSQL, SSH як ubuntu, деплой через CI/CD» — напишіть це в CLAUDE.md один раз. Ми зменшили wrong-approach інциденти вдвічі після документування інфраструктурних конвенцій.

### 2. Фокусовані сесії > марафони

2-3 пов'язані задачі за сесію. Не 10+ різних. Context window — не нескінченний, і якість деградує з кількістю переключень.

### 3. MCP — це серйозна інфраструктура

Підключіть Nextcloud/Linear для тасок, AWS MCP для інфраструктури, email MCP для комунікацій. Claude Code стає не просто кодером, а повноцінним операційним хабом.

### 4. Не пишіть детальні промпти

Запускайте Claude на задачу і коригуйте. Це швидше ніж 10 хвилин формулювання ідеального промпту. Claude адаптується до ваших корекцій.

### 5. Тести та TypeScript — ваша страховка

102 випадки buggy code — це нормально, якщо у вас є тести та type checking. Ми ловимо 90% помилок автоматично.

---

## Висновки

735 комітів за 25 днів — це не фантастика. Це результат системної роботи з AI-напарником, де:

- **CLAUDE.md** замінює онбординг
- **MCP-інтеграції** замінюють переключення між інструментами
- **Паралельні сесії** (41% повідомлень) замінюють чекання
- **TypeScript + тести** компенсують 102 випадки buggy code
- **Корекція в реальному часі** компенсує 106 wrong approaches

Чи замінить AI розробників? Ні. Але один розробник з правильно налаштованим AI-напарником — це вже не один розробник. Це маленька команда, яка ніколи не спить, не хворіє, і може паралельно деплоїти на прод, скрейпити 11M записів, та писати Telegram-бота з цитатами Бендера.

---

*P.S. Ця стаття теж написана за допомогою Claude Code. Meta? Можливо. Але 735 комітів — реальні.*
