...
Регистрация: [legal.org.ua](https://legal.org.ua)`,
  ...
  'workflow-memory-architecture': {
    title: 'Workflow Memory Architecture: как заменить CLAUDE.md на queryable memory layer',
    punchline: 'Каждая новая сессия Claude Code стартует с нуля: перечитывает файлы, накапливает лишний стейт и теряет контекст предыдущих решений. На горизонте 6+ недель ни одно контекстное окно — даже 1M токенов — не выдерживает. Мы проектируем трёхслойный workflow memory layer вместо плоских CLAUDE.md дампов.',
    readTime: '28 мин',
    content: `# Workflow Memory Architecture for Long-Horizon Composition

*Владимир Овчаров, Founder & CEO, LEX AI LLC*
...
Долгосрочная совместная работа (задачи, растянутые на недели и месяцы) сейчас ограничена не возможностями модели, а механикой контекстного окна. Каждая новая сессия Claude Code стартует с чистого листа, перечитывает файлы в контекст, накапливает избыточный стейт и теряет предыдущие рассуждения при закрытии сессии. На горизонте 6 недель рабочий набор релевантных предшествующих решений превышает то, что эффективно помещается в любое контекстное окно — даже при 1M токенов деградация внимания и стоимость инференса делают это неработоспособным.

Принятый обходной путь — файлы \`CLAUDE.md\`, дампы README, ручные саммари контекста — это плоское, ручное, с потерями сжатие состояния проекта. Оно фиксирует намерение, но не текстуру предыдущих решений: какие альтернативы были рассмотрены и отвергнуты, какие конституционные принципы были задействованы, какие правки исправили какие выходы валидаторов.

Мы предлагаем заменить плоские дампы контекста на **структурированный, адресуемый, запрашиваемый слой workflow memory**, из которого Claude Code извлекает данные при каждой сессии, в привязке к текущей задаче.

---

## 2. Архитектурный тезис

Workflow memory декомпозируется на три слоя, каждый со своей семантикой извлечения:

| Слой | Содержание | Сигнал для извлечения | Существующий субстрат |
| --- | --- | --- | --- |
| **Domain** | Юридические данные — законы, судебные решения, регламенты | Семантическое сходство с запросом | Qdrant (уже в продакшне) |
| **Workflow** | Архитектурные решения, определения валидаторов, контракты MCP-инструментов, реестр конституции, ADR-документы, история изменений с обоснованием | Гибрид: семантика + структура (компонент, слой, поверхность) | Новое: PostgreSQL + Qdrant коллекция |
| **Practitioner** | Следы правок, метки разрешения неоднозначностей, паттерны решений, история accept/reject по поверхностям, операционные принципы основателя | Гибрид: семантика + время + результат | Новое: расширение схемы rlhf-signals |

Новая задача для Claude Code не загружает "проект" — она выполняет **memory query**, который возвращает компактный, обусловленный задачей дайджест из трёх слоёв.

Архитектура **двухрежимная**: pull-based для быстрых циклов (хуки на старте сессии запрашивают память по требованию) и push-based для медленных циклов (плановый оркестратор обновляет записи памяти для долгосрочных задач пропорционально их активности). Push-режим — это то, что делает горизонты 6+ недель практически осуществимыми.
...
**Основные:**
- Сократить среднюю загрузку сессии Claude Code с ~25 чтений файлов + дампов до 1 memory query + 3–5 точечных чтений
- Продемонстрировать измеримую преемственность между сессиями: решение, принятое по Задаче A на неделе 2, корректно извлекается и учитывается по Задаче B на неделе 6
- Создать инструментированный слой workflow memory, который станет субстратом для секции RLHF-статьи о долгосрочной композиции

**Вторичные:**
- Сгенерировать четвёртый класс следов правок: *retrieval-correction* (случаи, где модель сделала бы X правильно, если бы релевантный контекст был извлечён)
- Запустить индекс practitioner layer, который автоматически фиксирует операционные паттерны основателя за 4–6 недель
...
Новый модуль рядом с существующим пайплайном rlhf-signals. Пайплайны ингеста для трёх слоёв, Retrieval API (HTTP + MCP tool обёртка), генерация эмбеддингов и переиндексация, гибридный язык запросов.

### 4.2 Domain Layer (интеграция существующего)

Существующие коллекции Qdrant с юридическими данными становятся пространством имён domain layer в едином retrieval API.

### 4.3 Workflow Layer (новая разработка)

Источники: реестр конституции, определения валидаторов, контракты MCP-инструментов, ADR-записи решений, git-история с семантическим обогащением, задачи Plane с переходами статусов.

### 4.4 Practitioner Layer (новая разработка)

Расширяет таблицу workflow_edits встраиваемыми артефактами — саммари следов, генерируемые ночным Bedrock-джобом по структурированным данным правок.

### 4.5 Memory Query API

Единый MCP-инструмент \`workflow_memory_query\`: эмбеддинг описания задачи → параллельный запрос трёх коллекций Qdrant → ре-ранкинг → сборка дайджеста в рамках токен-бюджета.

### 4.6 Интеграция с Claude Code

Хук на старте сессии подготавливает дайджест памяти. CLAUDE.md превращается из плоского дампа в инструкцию маршрутизации.

### 4.7 Инструментация промахов

Каждый запрос логируется. После закрытия сессии job реконсиляции обнаруживает потенциальные промахи — создавая класс retrieval-correction.

### 4.8 Оркестратор долгосрочных задач

Push-based оркестратор, который отслеживает задачи Plane с меткой \`LONG-TERM\` и обновляет записи памяти пропорционально активности. Отдельный Docker-контейнер с Bedrock-пайплайном суммаризации.

### 4.9 Prompt-Commit Bridge (Фаза 0 — уже работает)

Ещё до построения полной инфраструктуры (фазы 1.0–1.5) мы развернули минимальный сборщик данных для practitioner layer — **prompt-commit bridge**.

**Что это:** Хук \`UserPromptSubmit\` в Claude Code, который на каждом промпте создаёт orphan commit в bare git-репозитории через \`git commit-tree\`. Никаких внешних сервисов, никаких баз — только git-объекты в \`~/.claude/prompt-corpus.git\`, синхронизированные с приватным GitHub-репозиторием.

**Что фиксируется:** Каждая запись — JSON-блоб со структурированными метаданными:

\`\`\`json
{
  "timestamp": "2026-05-09T19:57:55+03:00",
  "prompt": "найди все решения ВС по делу о признании договора недействительным",
  "prompt_length": 88,
  "session_id": "a1b2c3...",
  "repo": "SecondLayer",
  "branch": "main",
...
1. **Захват** — хук читает JSON из stdin, фильтрует короткие промпты (<5 символов), собирает метаданные (ветка, репозиторий, режим разрешений)
2. **Хранение** — \`git hash-object\` → \`git mktree\` → \`git commit-tree\` с цепочкой родительских коммитов. Orphan-коммиты не загрязняют ни одну ветку
3. **Синхронизация** — автоматический push на GitHub каждые 10 захватов (фоновый, неблокирующий)
4. **Запросы** — CLI-инструмент для анализа: статистика, поиск, экспорт JSONL, группировка по репозиториям/сессиям

**Производительность:** 15 мс на захват — незаметно для пользователя при таймауте хука в 5 секунд.

**Зачем это нужно сейчас:** Статья описывает practitioner layer как то, что будет построено в фазе 1.2 (недели 5–6). Но сбор сырого корпуса промптов — это **предпосылка**, которая не требует никакой инфраструктуры из фаз 1.0–1.5. За 4–6 недель пассивного сбора это даёт:

- **Натуральный текстовый корпус** для disambiguation labels — без еженедельных ручных сессий аннотации
- **Операционные паттерны основателя** — частотное распределение тем, переключение между проектами, длина промптов как прокси для сложности задач
- **Стандартизированный протокол сбора данных** — масштабируемый на когорту в Phase 2 и цитируемый в arXiv-статье

Bridge намеренно разделяет захват (быстрый, локальный, приватный) и анализ (отдельный шаг, после накопления корпуса). Это та же логика, что и в секции 3.1 RLHF-статьи об implicit signal capture — только вместо edit-traces мы собираем prompt-traces.

---

## 5. Фазирование

- **Фаза 0** (неделя 0 — развёрнута): Prompt-Commit Bridge — хук, bare git repo, GitHub sync, CLI-инструмент ✅
- **Фаза 1.0** (недели 1–2): Фундамент — схема, коллекции Qdrant, базовый ингест
- **Фаза 1.1** (недели 3–4): Реестр конституции + бекфил ADR
- **Фаза 1.2** (недели 5–6): Запуск practitioner layer
- **Фаза 1.3** (неделя 7): Интеграция с Claude Code
- **Фаза 1.4** (неделя 8): Инструментация промахов извлечения
- **Фаза 1.5** (недели 9–11): Оркестратор долгосрочных задач + линейка инструментов
...
| --- | --- | --- |
| Среднее чтение файлов при старте | ~25 | ≤8 |
| Среднее кол-во входных токенов | ~30K+ | ≤10K |
| Точность извлечения принципов конституции | n/a | ≥80% |
| Релевантность следов practitioner | n/a | ≥70% |
| Задержка обновления долгосрочных задач | вручную или никогда | ≤24 часа |
| Покрытие линейки инструментов | ~0% | ≥80% |

---

*Владимир Овчаров — [LinkedIn](https://www.linkedin.com/in/vladimir-ovcharov/) | [volodymyr@legal.org.ua](mailto:volodymyr@legal.org.ua)*
*LEX AI LLC, Киев, Украина*

Регистрация: [legal.org.ua](https://legal.org.ua)`,
  },
  'tokenizer-fertility-zero-shot-ukrainian-legal': {
    title: 'Tokenizer Fertility и Zero-Shot Performance Foundation Models на украинском юридическом тексте',
    punchline: 'Мы сравнили 7 foundation models от 5 провайдеров на 300 судебных решениях из ЕГРСР: токенайзеры Llama генерируют на 60% меньше токенов чем Qwen, few-shot prompting деградирует результаты для 4 из 7 моделей, а размер модели — плохой предиктор качества на украинском языке. Nemotron Super 3 (120B) обходит Mistral Large 3 (675B) на всех задачах за треть цены.',
    readTime: '20 мин',
    content: `# Tokenizer Fertility и Zero-Shot Performance Foundation Models на украинском юридическом тексте: сравнительное исследование
...
Мы оценили семь foundation models от пяти провайдеров на украинском юридическом тексте, измеряя tokenizer fertility и downstream zero-shot performance на трёх задачах: классификация типа дела (4 класса), классификация результата рассмотрения (5 классов) и извлечение правовых норм (F1). Корпус оценки — 300 судебных решений (273 после валидации) из Единого государственного реестра судебных решений (ЕГРСР), стратифицированных по четырём юрисдикционным категориям.

Основные результаты: tokenizer fertility на украинском тексте варьируется в 1.6 раза между моделями. Llama 4 Maverick — самая эффективная токенизация (2.43 токена/слово), Qwen 3 — наименее эффективная (3.90 токена/слово). Разрыв в fertility напрямую транслируется в стоимость: модели Qwen 3 потребляют на 60% больше токенов на документ чем модели Llama при идентичном вводе.

**Ключевые слова:** tokenizer fertility, украинский NLP, классификация юридического текста, мультиязычная оценка LLM, foundation models, AWS Bedrock

---

## 1. Введение

Массовое распространение больших языковых моделей (LLM) создало неявную иерархию среди мировых языков. Английский, как доминирующий язык в корпусах предобучения, выигрывает от хорошо оптимизированных токенайзеров, обширных бенчмарков и тщательной оценки. Языки с кириллицей, сложной морфологией и меньшим цифровым следом — такие как украинский — сталкиваются с кумулятивным недостатком: их слова разбиваются на большее количество subword-токенов, что приводит к более высоким затратам на инференс, меньшему эффективному контекстному окну и потенциально деградированной производительности.

Для практиков, строящих legal tech платформы, которые должны обрабатывать десятки тысяч судебных решений ежедневно, выбор foundation model имеет прямые последствия для операционной стоимости, латентности и точности.
...
300 судебных решений из ЕГРСР, стратифицированных по юрисдикционной категории: гражданские, уголовные, хозяйственные, административные — по 75 решений каждой.

### 3.2 Модели

| Модель | Провайдер | Архитектура | Регион |
|--------|----------|-------------|--------|
...
1. **Классификация типа дела (4 класса)** — гражданское, уголовное, хозяйственное, административное.
2. **Классификация результата рассмотрения (5 классов)** — удовлетворено, отказано, оставлено без рассмотрения, частично удовлетворено, закрыто.
3. **Извлечение правовых норм (F1)** — извлечение всех правовых норм (закон + статья) в структурированном JSON.
...
| Модель | Fertility (\\u2193) | Chars/Token (\\u2191) | Std |
|--------|----------|------------|-----|
...
Токенайзеры Llama — самые эффективные на украинском тексте (2.43-2.65 токена/слово). Qwen — наименее эффективные (3.90 токена/слово) — на 60.3% выше чем Llama 4 Maverick.

### 4.2 Композитный рейтинг

| Модель | Type Acc% | Outc. Acc% | Norm F1 | Комп. балл | Стоимость |
|--------|----------|-----------|---------|-----------|----------|
| Nemotron Super 3 | 99.0 | 96.0 | 0.560 | 83.1 | $3.61 |
| Nova Pro | 98.0 | 92.3 | 0.590 | 82.7 | $4.98 |
| Llama 3.3 70B | 94.7 | 89.7 | 0.617 | 81.6 | $3.00 |
| Mistral Large 3 | 96.0 | 91.6 | 0.576 | 81.1 | $10.99 |
| Qwen3 235B | 97.7 | 93.8 | 0.479 | 79.2 | $5.37 |
| Llama 4 Maverick | 99.0 | 91.2 | 0.496 | 79.6 | $0.81 |
| Qwen3 32B | 95.7 | 86.8 | 0.531 | 77.8 | $2.64 |

Общая стоимость эксперимента по всем 7 моделям: **$31.41**.

### 4.3 Эффект деградации Few-Shot

12 из 21 комбинации модель-задача показывают деградацию при few-shot prompting. Особенно тяжёлый эффект для классификации результата рассмотрения: Qwen 3 235B теряет 26.0 процентных пункта, Nemotron Super 3 — 12.8 п.п.

---

## 5. Ключевые выводы

1. **Начинайте с анализа токенайзера.** Разница fertility в 1.6x компаундируется на каждом вызове инференса.
2. **По умолчанию — zero-shot.** Для морфологически богатых языков валидируйте few-shot против zero-shot для каждой модели и каждой задачи.
3. **Игнорируйте количество параметров.** Модель 120B обошла модель 675B на всех задачах.
4. **Рассматривайте мультимодельные архитектуры.** Роутинг лёгкой классификации на Maverick ($0.00045/call) и сложной экстракции на Llama 3.3 ($0.00167/call) оптимизирует и стоимость, и качество.

---

**Данные и код.** Код оценки и агрегированные результаты доступны на https://github.com/overthelex/rlhf-signals.
