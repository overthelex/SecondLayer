...
Один гражданский документ в среднем: 2.7 чанка × 850 токенов = 2300 токенов. При $0.06/1M токенов это **$0.000138 за документ**.

На 10% (3.37M документов) мы потратили $467.50 за 14.8 часа. Осталось 30.33M документов — это ещё примерно **$3,100** и **130 часов** (около 5.4 суток непрерывного прогона). Суммарная стоимость полной векторизации ГПК-когорты — около **$3,600**.
...
- ~$3,600 суммарная стоимость полной векторизации ГПК
...
Регистрация: [legal.org.ua](https://legal.org.ua)`,
  ...
  'opendata-sync-pipeline-engineering': {
    title: 'Как мы синхронизируем 340M+ записей из 15 государственных API, которые постоянно падают',
    punchline: 'Multi-IP импорт, автоматический scheduler, freshness-мониторинг — инженерия data pipeline для открытых данных Украины. От первого 404 до стабильного обновления 64 таблиц каждую ночь.',
    readTime: '15 мин',
    content: `# Как мы синхронизируем 340M+ записей из 15 государственных API, которые постоянно падают

Когда строишь юридическую AI-платформу на открытых данных Украины, главный вызов — не AI и не поиск. Это **надёжное получение данных** из десятков государственных источников, каждый из которых имеет свои ограничения, форматы и проблемы со стабильностью.

Эта статья — инженерный разбор того, как мы построили полностью автоматизированный pipeline синхронизации для 340+ миллионов записей из 15 источников. От архитектуры multi-IP импорта до cron-scheduler'а и системы мониторинга freshness.
...
└────────┬──────────┘ └────────┬─────────┘
         ...
         ▼                     ▼
┌─────────────────────────────────────────┐
│  PostgreSQL: 64 opendata_* таблицы      │
...
|-------|-----------|----------------|
| 03:00 ежедневно | МВД розыск, МВД пропавшие, МВД авто, НАЗК коррупционеры | backend |
...
|---------|--------|---------|
| Ежедневно (1д) | 18 | Розыск МВД, коррупционеры НАЗК, должники, исполнительные |
| Еженедельно (7д) | 42 | Патенты, марки, адвокаты, депутаты, судьи, законопроекты |
| Ежемесячно (30д) | 4 | Графики заседаний, статусы дел |
...
\`\`\`
═══════════════════════════════════════════════════════════════
  📦 SecondLayer (основная) — 64 таблицы, 247.3 GB всего
═══════════════════════════════════════════════════════════════
  #   Таблица                            Строк  Размер  Норма  Давность
  ──────────────────────────────────────────────────────────────────────
  1   opendata_wanted_persons             71K   85 MB    1д   2мин назад 🟢
  2   opendata_missing_persons           112K  138 MB    1д   12мин назад🟢
  3   opendata_corruption                 58K   42 MB    1д   3ч назад   🟢
  4   opendata_patents                   119K  180 MB    7д   2д назад   🟢
  5   spending_acts                     9.45M  8.2 GB    7д   5д назад   🟢
  ...
\`\`\`
...
| Метрика | Значение |
|---------|---------|
| Общий объём данных | 340M+ записей, 247 GB |
| Количество источников | 15 автоматизированных |
| Количество таблиц | 64 opendata-таблицы |
| MCP-инструментов для поиска | 26 (opendata + spending + registries) |
| Ежедневная синхронизация | 8 источников (03:00–05:00 UTC) |
| Еженедельная синхронизация | 11 источников (выходные) |
| Параллельных соединений | до 50 (10 IP × 5 потоков) |
| Время полного импорта УИПВ | ~45 мин (182K записей) |
...
1. **Alerting** — Telegram-бот для уведомлений о failed imports вместо ручной проверки db-status
2. **Incremental sync** — для источников, поддерживающих \`modified_since\`, загружать только изменения
3. **Data quality checks** — автоматическая проверка: если количество записей упало >20% после импорта, что-то пошло не так
4. **Больше источников** — ДРРП (реестр недвижимости), Минюст (реестр обременений), ГФС (налоговые должники)

---

## Вывод

Построить pipeline для открытых данных Украины — это не про \`fetch → insert\`. Это про инженерию надёжности: retry, rate limit, multi-IP, мониторинг freshness, graceful degradation.

Каждый из 15 источников — это отдельная история с уникальными проблемами. Но когда pipeline работает стабильно, юрист задаёт вопрос в чат и получает актуальные данные из МВД, НАЗК, УИПВ, НАИС и spending.gov.ua — даже не задумываясь, сколько инженерной работы стоит за каждым ответом.
...
Регистрация: [legal.org.ua](https://legal.org.ua)`,
  ...
  'claude-code-building-startups': {
    title: 'Как я написал 735 коммитов за 25 дней: Claude Code как полноценный инженерный напарник',
    punchline: '486 сессий, 5 612 сообщений, 193 000 строк кода, 22 326 bash-команд — и ни одного нанятого разработчика. Реальная статистика использования Claude Code для построения legal tech платформы, data pipelines на 340M+ записей и MCP-интеграций.',
    readTime: '12 мин',
    content: `# Как я написал 735 коммитов за 25 дней: Claude Code как полноценный инженерный напарник
...
## Цифры за 25 дней (18 марта — 12 апреля 2026)

| Метрика | Значение |
|---------|----------|
| Сессий | 486 (285 проанализировано) |
| Сообщений | 5 612 |
| Коммитов | 735 |
| Строк написано | +193 340 |
| Строк удалено | -14 259 |
| Файлов изменено | 1 811 |
| Bash-команд | 22 326 |
| Edit-операций | 3 782 |
| Sub-агентов | 864 |
| Параллельных сессий | 41% сообщений |

Это не теоретическая продуктивность. Это реальный git log.

---

## Что именно я строил

### 1. Legal Tech платформа (28 сессий)

Основной продукт: баг-фиксы, новые фичи (Diia-аутентификация, контракты разработчиков, email-уведомления, испанская локализация с geo-detection), UI-редизайн, 93+ тестов.

Claude Code работал как full-stack разработчик: мультифайловые изменения, создание PR, мердж, деплой — всё в одной сессии.

### 2. Data pipelines для открытых данных (18 сессий)
...
### 3. Инфраструктура и DevOps (16 сессий)

EC2 provisioning в разных регионах (Париж, Испания), CI/CD pipeline фиксы, blue-green деплой с preview-средой, Docker/nginx дебагинг, миграции серверов.

### 4. MCP Server Ecosystem (14 сессий)

Построение и конфигурация MCP-серверов для Nextcloud Deck/Tables, Thunderbird email и ChatGPT. Миграция 180 задач из Linear в Nextcloud Deck.

### 5. Контент и side-проекты (8 сессий)

Инвестор-мемо, юридические документы, блог-статьи, и Telegram-бот с цитатами Бендера из Футурамы.

---

## Как выглядит типичная рабочая сессия

Я не пишу детальные промпты. Мой стиль — **запускаю Claude на задачу, смотрю что делает, корректирую курс в реальном времени**.

106 случаев когда Claude начинал с неправильного подхода, которые я быстро перенаправлял. Это не баг — это особенность работы с AI-напарником.

**Результат: 89% сессий завершились successfully.**
...
Nextcloud Deck для задач, AWS API для инфраструктуры, Thunderbird для email — Claude Code становится полноценным операционным хабом.

---

## Где не работает (честно)

### Wrong Approach — 106 случаев

Claude часто начинает с неправильного подхода: ищет не в том каталоге, использует SSH вместо MCP tools, выбирает медленную стратегию для DB-операций.

### Buggy Code — 102 случая

Код с первого раза работает не всегда. Но с TypeScript и тестами это ловится быстро.
...
735 коммитов за 25 дней — это не фантастика. Это результат системной работы с AI-напарником, где CLAUDE.md заменяет онбординг, MCP-интеграции заменяют переключение между инструментами, а TypeScript + тесты компенсируют ошибки.

Заменит ли AI разработчиков? Нет. Но один разработчик с правильно настроенным AI-напарником — это уже не один разработчик.
