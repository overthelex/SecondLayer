 ...
 * Transforms raw tool responses into standardized Decision[], Citation[], VaultDocument[].
 */
...
// ---- Extractors by tool category ----

function extractCourtEvidence(toolName: string, parsed: any): EvidenceResult {
...
function extractLegislationEvidence(toolName: string, parsed: any): EvidenceResult {
...
function extractProceduralNormEvidence(toolName: string, rawResult: any): EvidenceResult {
  const citations: Citation[] = [];
...
function extractVaultEvidence(toolName: string, parsed: any): EvidenceResult {
  const citations: Citation[] = [];
...
function extractRetrieveLegalSourcesEvidence(toolName: string, parsed: any): EvidenceResult {
...
function extractRadaEvidence(toolName: string, parsed: any): EvidenceResult {
  const citations: Citation[] = [];
  const documents: VaultDocument[] = [];
  const radaTools = [
...
function extractRegistryEvidence(toolName: string, parsed: any): EvidenceResult {
  const citations: Citation[] = [];
  const documents: VaultDocument[] = [];
  const registryTools = [
...
function extractProceduralEvidence(toolName: string, parsed: any): EvidenceResult {
  const citations: Citation[] = [];
...
function extractDDEvidence(toolName: string, parsed: any): EvidenceResult {
  const citations: Citation[] = [];
  const ddTools = [
    'generate_dd_report',
    'risk_scoring',
