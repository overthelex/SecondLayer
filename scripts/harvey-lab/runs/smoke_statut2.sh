...
uv run --with "anthropic[bedrock]" python -m evaluation.run_eval \
    ...
    --parallel 6 2>&1 | grep -E "criteria passed|Score:" | tail -2 > ~/harness-lab/smoke_statut2.log
echo SMOKE_STATUT2_DONE >> ~/harness-lab/smoke_statut2.log
cat ~/harness-lab/smoke_statut2.log
