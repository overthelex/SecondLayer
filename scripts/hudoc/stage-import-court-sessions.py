#!/usr/bin/env python3
"""
Stage server import of court session revisions (526-1052).
Streaming: download one CSV at a time due to limited disk (3.5GB free).
...
DB_CONTAINER = "secondlayer-postgres-stage"
DB_USER = "secondlayer"
...
def fetch_revisions():
    ...
    # Download to file first (large response can get truncated in pipe)
    ...
    for attempt in range(DEADLOCK_RETRIES):
        try:
            ...
            if "deadlock" in err_msg.lower() or "lock timeout" in err_msg.lower():
                ...
                raise
    raise RuntimeError(f"Lock conflict persisted after {DEADLOCK_RETRIES} retries for {revision_date}")


def setup_tables(num_workers):
    print("[*] Preparing stage database tables...")
...
def main():
    ...
    # Stage: limited disk, use 4 workers (safe for 3.5GB free)
    parser.add_argument("--workers", type=int, default=4)
    parser.add_argument("--start", type=int, default=526)
    ...
    print(f"[*] Download size: {slice_gb:.1f} GB (streaming, ~300MB on disk at a time)")
    ...
    print(f"[*] Current rows in stage court_sessions: {current_count:,}")
