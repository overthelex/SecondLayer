#!/usr/bin/env python3
"""
Second pass (LOCAL): download truncated days from spending.gov.ua.
Reads data/truncated_days.txt, splits each day by documentDate windows.
Uses 3 local IPs × 2 threads each (6 workers).
Saves to local data/ dir — rsync to prod after.
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(os.path.dirname(os.path.abspath(__file__)), "spending_pass2_local.log")),
    ],
)
...
_dns_cache = {}
_dns_lock = Lock()
stats = {"done": 0, "skipped": 0, "failed": 0, "documents": 0, "requests": 0}
stats_lock = Lock()
...
def process_day(doc_type, sign_str, ip):
    ...
    # 6-month lookback in monthly windows — parallel
    cursor = sign_date
...
def main():
    ...
    # Create output dirs
    for t in set(t[0] for t in tasks):
        os.makedirs(os.path.join(OUTPUT_DIR, t), exist_ok=True)

    threads_per_ip = int(sys.argv[1]) if len(sys.argv) > 1 else 2
    log.info(f"LOCAL pass2: {total} truncated days, {len(SOURCE_IPS)} IPs x {threads_per_ip} threads = {len(SOURCE_IPS) * threads_per_ip} workers")
    ...
    log.info(f"Completed: {stats['done']} | Skipped: {stats['skipped']} | Failed: {stats['failed']} | "
