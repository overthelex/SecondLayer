#!/usr/bin/env python3
"""
Batch vectorization of КУпАП (justice_kind=5) court decisions.
...
Usage:
    python3 vectorize-kupap-batch.py [--concurrency 35] [--batch-size 128] [--dry-run]
"""
...
def get_next_voyage_key() -> str:
    """Round-robin across available VoyageAI API keys."""
...
def chunk_text(text: str) -> list[str]:
    """Split text into chunks of ~MAX_CHUNK_CHARS with overlap."""
    ...
    while pos < len(text):
        ...
        if new_pos <= pos:
            new_pos = pos + MAX_CHUNK_CHARS // 2  # safety: always advance
        pos = new_pos
...
def embed_texts(texts: list[str], max_retries: int = 5) -> tuple[list[list[float]], int]:
    """Embed texts via VoyageAI with round-robin API keys. Returns (embeddings, total_tokens)."""
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            if e.code == 429:
                # Switch to other key on rate limit
                api_key = get_next_voyage_key()
...
def qdrant_scroll_doc_ids(justice_kind: int = 5, batch_size: int = 1000) -> set[int]:
    """Get all edrsr_doc_ids already in Qdrant for given justice_kind."""
    log.info("Loading existing doc_ids from Qdrant...")
    ...
    log.info(f"Found {len(doc_ids):,} already vectorized doc_ids in Qdrant")
    return doc_ids
...
def pg_connect():
    ...
    # Set statement timeout to avoid long-running queries killing the connection
    with conn.cursor() as cur:
        cur.execute("SET statement_timeout = '120s'")
    return conn

def pg_reconnect(conn):
    """Close and reconnect to PG."""
    try:
        conn.close()
    except Exception:
        pass
...
def pg_count_kupap(conn) -> int:
    """Count total КУпАП docs with fulltext (fast estimate)."""
    with conn.cursor() as cur:
        # Fast: use reltuples estimate instead of full COUNT
        cur.execute("""
            SELECT COALESCE(
                (SELECT reltuples::bigint FROM pg_class WHERE relname = 'edrsr_documents'),
                0
            )
        """)
        total_est = cur.fetchone()[0]
        # КУпАП (justice_kind=5) is ~11% of total based on prior analysis
        # Use known value from earlier check: 10,887,131
        return 10_887_131

def pg_fetch_batch(conn, last_doc_id: int, batch_size: int) -> list[dict]:
    """Fetch next batch of КУпАП docs after last_doc_id.

    Fetches metadata first (fast), then fulltext in small sub-batches
    to avoid PG connection drops on huge result sets.
    """
    with conn.cursor() as cur:
        # Step 1: fetch doc_ids + metadata (lightweight, no full_text)
        cur.execute("""
    ...
    # Step 2: fetch full_text in sub-batches of 500
    FT_BATCH = 500
    ...
    # Return only docs that have full_text
    return [d for d in docs if d.get("full_text")]

# ── Worker: process one batch ───────────────────────────────────────────────

def process_batch(docs: list[dict], progress: Progress, dry_run: bool = False) -> int:
    """Process a batch of docs: chunk → embed → upsert. Returns last doc_id."""
    if not docs:
        return 0

    # Chunk all docs
    all_chunks = []  # (doc, chunk_text, chunk_index, total_chunks)
    for doc in docs:
        text = doc["full_text"]
    ...
    # Embed in sub-batches of 128 texts
    EMBED_BATCH = 128
    points = []
    total_tokens = 0
    retries_429 = 0
    ...
    # Upsert to Qdrant in sub-batches of 100
...
CHECKPOINT_FILE = "/tmp/vectorize-kupap-checkpoint.json"
...
def main():
    parser = argparse.ArgumentParser(description="Batch vectorize КУпАП decisions")
    parser.add_argument("--concurrency", type=int, default=35, help="Concurrent embedding threads")
    parser.add_argument("--batch-size", type=int, default=500, help="Docs per PG fetch batch")
    parser.add_argument("--embed-batch", type=int, default=128, help="Docs per VoyageAI request")
    ...
    log.info(f"Config: concurrency={args.concurrency}, batch={args.batch_size}, model={VOYAGE_MODEL}, api_keys={len(_voyage_keys)}")
    ...
    # Connect to PG
    conn = pg_connect()
    total = pg_count_kupap(conn)
    log.info(f"Total КУпАП docs with fulltext: {total:,}")
    ...
    # Skip existing docs in Qdrant
    existing_ids = set()
    if args.skip_existing:
        existing_ids = qdrant_scroll_doc_ids(justice_kind=5)

    progress = Progress(total_docs=total)
    ...
    try:
        while True:
            # Fetch a big batch from PG (with reconnect on failure)
            try:
                pg_batch = pg_fetch_batch(conn, last_doc_id, min(args.batch_size * args.concurrency, 2000))
            except Exception as e:
            ...
            if not pg_batch:
                last_doc_id = pg_batch[-1]["doc_id"] if pg_batch else last_doc_id + args.batch_size * args.concurrency
                continue
            ...
            with ThreadPoolExecutor(max_workers=args.concurrency) as pool:
                ...
                for future in as_completed(futures):
                    try:
                        batch_last_id = future.result()
                        if batch_last_id > last_doc_id:
                            last_doc_id = batch_last_id
                    except Exception as e:
                        log.error(f"Worker error: {e}")
                        progress.add(err_other=1)

            # Update last_doc_id to max in this PG batch
